...
* `options` - Optional configuration:
  ...
  * `mergeConfig` - When `false`, the config object or file you provide is used as-is without being merged with any `superstatic.json` or `firebase.json` found on disk. Defaults to `true`. Can also be set via the `--no-merge-config` CLI flag.
